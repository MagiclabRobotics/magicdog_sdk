...
| Python 依赖 | 见 `requirements.txt`：`numpy`、`matplotlib`（地图）；`opencv-python`、`Pillow`（Video RTSP）；`ultralytics`（YOLO 检测，可选） |
...
| 人体(DNN) | OpenCV DNN | MobileNet-SSD；失败时自动降级 HOG 行人检测 |

YOLO 模式每 4 帧推理一次以减轻 CPU 负载；统计栏显示类别与数量（如 `person×1, cup×2` 或 `人体×1 [HOG]`）。
...
| `MobileNetSSD_deploy.prototxt` | 人体 DNN | 首次自动下载到 `models/` |
| `MobileNetSSD.caffemodel` | 人体 DNN（约 23 MB） | 首次自动下载到 `models/` |
...
| `VIZ_NAV_DNN_PROTOTXT` | MobileNet-SSD prototxt 路径 |
| `VIZ_NAV_DNN_CAFFEMODEL` | MobileNet-SSD caffemodel 路径 |
...
- [MobileNetSSD_deploy.caffemodel](https://github.com/Sujan-Roy/Real-Time-Object-detection-with-MobileNet-and-SSD/raw/main/MobileNetSSD_deploy.caffemodel)
...
| 人体(DNN) 无检测 | 检查 `models/` 下 caffemodel 约 23 MB；失败时会显示 `[HOG]` 降级 |
